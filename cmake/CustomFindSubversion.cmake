...
if(Subversion_SVN_EXECUTABLE)
  # the subversion commands should be executed with the C locale, otherwise the message (which are parsed) may be
  # translated, Alex
  ...
  if(_Subversion_VERSION_RESULT EQUAL 0)
    ...
    if(_Subversion_VERSION_STDERR MATCHES
  ...
  macro(Subversion_WC_INFO dir prefix)
    ...
    cmake_parse_arguments("Subversion_WC_INFO" "IGNORE_SVN_FAILURE" "" "" ${ARGN})
    ...
    if(${Subversion_svn_info_result} EQUAL 0)
      ...
      string(REGEX REPLACE "^(.*\n)?Last Changed Author: ([^\n]+).*" "\\2" ${prefix}_WC_LAST_CHANGED_AUTHOR
                           "${${prefix}_WC_INFO}")
      string(REGEX REPLACE "^(.*\n)?Last Changed Rev: ([^\n]+).*" "\\2" ${prefix}_WC_LAST_CHANGED_REV
                           "${${prefix}_WC_INFO}")
      string(REGEX REPLACE "^(.*\n)?Last Changed Date: ([^\n]+).*" "\\2" ${prefix}_WC_LAST_CHANGED_DATE
                           "${${prefix}_WC_INFO}")
    endif()
  ...
  macro(Subversion_WC_LOG dir prefix)
    # This macro can block if the certificate is not signed: svn ask you to accept the certificate and wait for your
    # answer This macro requires a svn server network access (Internet most of the time) and can also be slow since it
    # access the svn server
    execute_process(
      COMMAND ${Subversion_SVN_EXECUTABLE} --non-interactive log -r BASE ${dir}
    ...
    if(NOT ${Subversion_svn_log_result} EQUAL 0)
      message(
        SEND_ERROR
          "Command \"${Subversion_SVN_EXECUTABLE} log -r BASE ${dir}\" failed with output:\n${Subversion_svn_log_error}"
      )
    endif()
  endmacro()
...
find_package_handle_standard_args(
  Subversion
  REQUIRED_VARS Subversion_SVN_EXECUTABLE
