...
if(${CMAKE_BUILD_TYPE} STREQUAL "Release")
  ...
  execute_process(
    COMMAND "sh" "${PROJECT_SOURCE_DIR}/scripts/ldd-copy-dependencies.sh" "-b"
            "${CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELEASE}/${PROJECT_NAME}" "-t" "${PROJECT_SOURCE_DIR}/dist/temp")

  # Create makeself package
  execute_process(
    COMMAND
      "sh" "${PROJECT_SOURCE_DIR}/scripts/makeself/makeself.sh" "--notemp" "--sha256" "${PROJECT_SOURCE_DIR}/dist/temp"
      "${PROJECT_NAME}.${PROJECT_VERSION_MAJOR}.${PROJECT_VERSION_MINOR}.${PROJECT_VERSION_PATCH}.run"
      "${PROJECT_NAME}")

  # Move to another directory
  execute_process(
    COMMAND "mv" "-f" "${PROJECT_NAME}.${PROJECT_VERSION_MAJOR}.${PROJECT_VERSION_MINOR}.${PROJECT_VERSION_PATCH}.run"
            "${PROJECT_SOURCE_DIR}/dist/makeself/")

  # Calculate MD5 checksums
  message(STATUS "${BoldBlue}Calculating MD5 checksums for binaries${ColourReset}")
  execute_process(
    COMMAND "find" "-type" "f" "-exec" "md5sum" "{}" "+"
  ...
  execute_process(
    COMMAND "find" "-type" "f" "-exec" "sha1sum" "{}" "+"
    WORKING_DIRECTORY "${PROJECT_SOURCE_DIR}/dist/temp"
  ...
  message(STATUS "${BoldBlue}${PROJECT_NAME}.${PROJECT_VERSION_MAJOR}.${PROJECT_VERSION_MINOR}"
                 ".${PROJECT_VERSION_PATCH}.run generated${ColourReset}")
endif()
