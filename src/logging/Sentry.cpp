	...
	void setNetworkContext()
	{
		...
		for (const ifaddrs *ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next)
		{
			...
			case AF_INET: {
				...
				inet_ntop(AF_INET, &(std::bit_cast<sockaddr_in *>(ifa->ifa_addr))->sin_addr, host.data(),
						  INET_ADDRSTRLEN);
			...
			case AF_INET6: {
				...
				inet_ntop(AF_INET6, &(std::bit_cast<sockaddr_in6 *>(ifa->ifa_addr))->sin6_addr, host.data(),
						  INET6_ADDRSTRLEN);
			...
			case AF_PACKET: {
				...
				if (const auto *sock = std::bit_cast<sockaddr_ll *>(ifa->ifa_addr);
					std::format_to_n(host.data(), MAC_LEN, "{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
									 sock->sll_addr[0], sock->sll_addr[1], sock->sll_addr[2], sock->sll_addr[3],
									 sock->sll_addr[4], sock->sll_addr[5])
						.size > 0)
				{
